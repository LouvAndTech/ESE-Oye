...
public class UserTable implements ITable {
    ...
    public void deleteUserAccount(String userSecureID) {
        try {
            new DatabaseRequest(factory, credentials, true).deleteValues(getTableName(), "secure_id=?", List.of(new Tuple<>(userSecureID, Types.VARCHAR)));
        } catch (SQLException e) {
    ...
    public Ternary isUserLocked(String userSecureID) {
        try {
            final ResultSetWrappingSqlRowSet res = new DatabaseRequest(factory, credentials, true).getValues(getTableName(), Arrays.asList("secure_id", "state"), "secure_id=?", List.of(new Tuple<>(userSecureID, Types.VARCHAR)));
    ...
    public void manageLockForUser(String userSecureID, boolean lockUser) {
        try {
            ...
            new DatabaseRequest(factory, credentials, true).updateValues(getTableName(), List.of("state"), List.of(userState), "secure_id=?", List.of(new Tuple<>(userSecureID, Types.VARCHAR)));
        } catch (SQLException e) {
    ...
    public String checkUserConnection(String mail, String password) {
        try {
            ResultSetWrappingSqlRowSet res = new DatabaseRequest(factory, credentials, true).getValues(getTableName(), Arrays.asList("mail", "password", "secure_id"), "mail = ?", List.of(new Tuple<>(mail, Types.VARCHAR)));
    ...
    public User getUser(String userSecureID) {
        ...
        try {
            ResultSetWrappingSqlRowSet res = new DatabaseRequest(factory, credentials, true).getValues(
                    ...
                    "secure_id=?",
                    List.of(new Tuple<>(userSecureID, Types.VARCHAR)));
    ...
    public boolean setImage(SFTPConnection ftpConnection, String userSecureID, InputStream inputstream) {
        ...
        try {
            request = new DatabaseRequest(factory, credentials);
            final ResultSetWrappingSqlRowSet requestUserDatabaseId = request.getValues(getTableName(), List.of("id"), "secure_id = ?", List.of(new Tuple<>(userSecureID, Types.VARCHAR))); //Get the id of the user and store it for the creation of the image
            ...
            ResultSetWrappingSqlRowSet requestExistingPic = request.getValues(USER_IMAGE_TABLE_NAME, Arrays.asList("id", "secure_id", "user"), "user=?", List.of(new Tuple<>(userDatabaseID, Types.INTEGER)));
            if (requestExistingPic.next()) {
                existingPicSecureID = requestExistingPic.getString("secure_id");
                request.deleteValues(USER_IMAGE_TABLE_NAME, "secure_id=?", List.of(new Tuple<>(existingPicSecureID, Types.VARCHAR)));
            }
            ...
            request.insertValues("User_IMG", List.of("user, secure_id"), Arrays.asList(userDatabaseID, imageID));
    ...
    public String getImage(String userSecureID) {
        ...
        try {
            ...
            final int userDatabaseId = request.getValues(getTableName(), List.of("id"), "secure_id = ", List.of(new Tuple<>(userSecureID, Types.VARCHAR))).getInt("id"); //Get the id of the user and store it to get the image

            final ResultSetWrappingSqlRowSet res = request.getValues(USER_IMAGE_TABLE_NAME, List.of("secure_id"), "user = ?", List.of(new Tuple<>(userDatabaseId, Types.INTEGER)));
            if (res.next())
    ...
    public boolean setNameSurname(String secureID, String newName, String newSurname) {
        try {
            new DatabaseRequest(factory, credentials, true).updateValues(getTableName(), Arrays.asList("name", "surname"), Arrays.asList(newName, newSurname), "secure_id = ?", List.of(new Tuple<>(secureID, Types.VARCHAR)));
            return true;
        } catch (SQLException e) {
            throw new DataCreationException(getClass(), CreationExceptionReason.FAILED_DB_CREATION);
        }
    ...
    public Tuple<String, String> getNameSurname(String secureID) {
        try {
            ResultSetWrappingSqlRowSet res = new DatabaseRequest(factory, credentials, true).getValues(getTableName(), Arrays.asList("name", "surname"), "secure_id = ?", List.of(new Tuple<>(secureID, Types.VARCHAR)));
            if (res.next())
    ...
    public boolean setPassword(String secureID, String newPassword) {
        try {
            new DatabaseRequest(factory, credentials, true).updateValues(getTableName(), List.of("password"), Collections.singletonList(newPassword), "secure_id = ?", List.of(new Tuple<>(secureID, Types.VARCHAR)));
            return true;
        } catch (SQLException e) {
            throw new DataCreationException(getClass(), CreationExceptionReason.FAILED_DB_CREATION);
        }
    ...
    public String getPassword(String secureID) {
        try {
            ResultSetWrappingSqlRowSet res = new DatabaseRequest(factory, credentials, true).getValues(getTableName(), List.of("password"), "secure_id = ?", List.of(new Tuple<>(secureID, Types.VARCHAR)));
            if (res.next())
    ...
    public boolean setPhoneNumber(String secureID, String newNumber) {
        ...
        try {
            ...
            if (request.getValuesCount(getTableName(), List.of("phone"), "phone=?", List.of(new Tuple<>(newNumber, Types.VARCHAR))) == 0) {
                request.updateValues(getTableName(), List.of("phone"), List.of(newNumber), "secure_id = ?", List.of(new Tuple<>(secureID, Types.VARCHAR)));
                return true;
            } else return false;
        } catch (SQLException e) {
            throw new DataCreationException(getClass(), CreationExceptionReason.FAILED_DB_CREATION);
    ...
    public String getPhoneNumber(String secureID) {
        try {
            ResultSetWrappingSqlRowSet res = new DatabaseRequest(factory, credentials, true).getValues(getTableName(), List.of("phone"), "secure_id = ?", List.of(new Tuple<>(secureID, Types.VARCHAR)));
            if (res.next())
    ...
    public boolean setBirthDate(String secureID, String newBirthDate) {
        try {
            new DatabaseRequest(factory, credentials, true).updateValues(getTableName(), List.of("birth"), Collections.singletonList(newBirthDate), "secure_id = ?", List.of(new Tuple<>(secureID, Types.VARCHAR)));
            return true;
        } catch (SQLException e) {
            throw new DataCreationException(getClass(), CreationExceptionReason.FAILED_DB_CREATION);
        }
    ...
    public Date getBirthDate(String secureID) {
        try {
            ResultSetWrappingSqlRowSet res = new DatabaseRequest(factory, credentials, true).getValues(getTableName(), List.of("birth"), "secure_id = ?", List.of(new Tuple<>(secureID, Types.VARCHAR)));
            if (res.next())
    ...
    public boolean setMail(String secureID, String newMail) {
        ...
        try {
            ...
            if (request.getValuesCount(getTableName(), List.of("mail"), "mail=?", List.of(new Tuple<>(newMail, Types.VARCHAR))) == 0) {
                request.updateValues(getTableName(), List.of("mail"), Collections.singletonList(newMail), "secure_id = ?", List.of(new Tuple<>(secureID, Types.VARCHAR)));
                return true;
            } else return false;
        } catch (SQLException e) {
            throw new DataCreationException(getClass(), CreationExceptionReason.FAILED_DB_CREATION);
    ...
    public String getMail(String secureID) {
        try {
            ResultSetWrappingSqlRowSet res = new DatabaseRequest(factory, credentials, true).getValues(getTableName(), List.of("mail"), "secure_id = ?", List.of(new Tuple<>(secureID, Types.VARCHAR)));
            if (res.next())

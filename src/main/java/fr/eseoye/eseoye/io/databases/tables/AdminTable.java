...
public class AdminTable implements ITable {
    ...
    public boolean deleteAdminAccount(String adminSecureID) {
        try {
            new DatabaseRequest(factory, credentials, true).deleteValues(getTableName(), "secure_id=?", List.of(new Tuple<>(adminSecureID, Types.VARCHAR)));
            return true;
        } catch (SQLException e) {
            return false;
        }
    ...
    public Tuple<String, String> getNameSurname(String adminSecureID) {
        try {
            ResultSetWrappingSqlRowSet res = new DatabaseRequest(factory, credentials, true).getValues(getTableName(), Arrays.asList("name", "surname"), "secure_id = ?", List.of(new Tuple<>(adminSecureID, Types.VARCHAR)));
            if (res.next())
    ...
    public Ternary isAnAdminSecureID(String secureID) {
        try {
            return new DatabaseRequest(factory, credentials, true).getValuesCount(getTableName(), List.of("secure_id"), "secure_id=?", List.of(new Tuple<>(secureID, Types.VARCHAR))) != 0 ? Ternary.TRUE : Ternary.FALSE;
        } catch (SQLException e) {

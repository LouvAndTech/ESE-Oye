...
public class PostTable implements ITable {
    ...
    public String createNewPost(SFTPConnection sftpConnection, String userSecureID, String title, String content, float price, int categoryID, int stateID, List<InputStream> images) throws DataCreationException {
        ...
        try {
            ...
            final ResultSetWrappingSqlRowSet requestUserDatabaseId = request.getValues(USER_TABLE_NAME, List.of("id"), "secure_id=?", List.of(new Tuple<>(userSecureID, Types.VARCHAR))); //Get the id of the user and store it for the creation of the post
    ...
    public boolean deletePost(SFTPConnection sftp, String postSecureID) {
        try {
            ...
            new DatabaseRequest(factory, credentials, true).deleteValues(getTableName(), "secure_id=?", List.of(new Tuple<>(postSecureID, Types.VARCHAR)));
            return true;
    ...
    public boolean setValidationStatus(String postSecureID, boolean newStatus) {
        try {
            new DatabaseRequest(factory, credentials, true).updateValues(getTableName(), List.of("lock"), List.of(newStatus), "`secure_id`=?", List.of(new Tuple<>(postSecureID, Types.VARCHAR)));
            return true;
        } catch (SQLException e) {
            e.printStackTrace();
            return false;
    ...
    public boolean modifyPost(String postSecureID, @Nullable String newTitle, @Nullable String newContent) {
        try {
            ...
            new DatabaseRequest(factory, credentials, true).updateValues(getTableName(), fields, values, "secure_id=?", List.of(new Tuple<>(postSecureID, Types.VARCHAR)));
        } catch (SQLException e) {
            return false;
        }
        return true;
        ...
        try {
            ...
            while (res.next()) {
                ...
                if (postImages.isEmpty())
                    postImages.add(SFTPHelper.getFormattedImageURL(ImageDirectory.ROOT, "", "404"));
    ...
    private Tuple<String, List<Tuple<Object, Integer>>> generateWhereClausePost(FetchPostFilter parameters) {
        ...
        if (parameters.isUserIDPresent()) {
            sb.append(USER_TABLE_NAME + ".secure_id=? AND ");
            whereObj.add(new Tuple<>(parameters.getUserSecureID(), Types.VARCHAR));
        }
        if (parameters.isCategoryPresent()) {
            sb.append(CATEGORY_TABLE_NAME + ".id=? AND ");
            whereObj.add(new Tuple<>(parameters.getCategoryID(), Types.INTEGER));
        }
        if (parameters.isStatePresent()) {
            sb.append(POST_STATE_TABLE_NAME + ".id=? AND ");
            whereObj.add(new Tuple<>(parameters.getStateID(), Types.INTEGER));
        }
        if (parameters.isMaxPricePresent()) {
            sb.append(getTableName() + ".price<=? AND ");
            whereObj.add(new Tuple<>(parameters.getMaxPrice(), Types.DECIMAL));
        }
        if (parameters.mustBeValidated()) {
            sb.append(getTableName() + ".lock=? AND ");
            whereObj.add(new Tuple<>(false, Types.BOOLEAN));
        }
    ...
    public String fetchLastLockedPostID() {
        ...
        try {
            ...
            final ResultSetWrappingSqlRowSet res = request.getValuesWithCondition("SELECT secure_id AS post_sid, `lock` FROM " + getTableName() + " " +
                    "WHERE " + getTableName() + ".lock=? ORDER BY " + getTableName() + ".id DESC", List.of(new Tuple<>(true, Types.BOOLEAN)));
    ...
    public PostComplete fetchEntirePost(String postID) {
        ...
        try {
            ...
            final ResultSetWrappingSqlRowSet res = request.getValuesWithCondition("SELECT " + getTableName() + ".id AS post_id, " + getTableName() + ".secure_id AS post_sid, " + getTableName() + ".title AS post_title, " + getTableName() + ".content AS post_content, " + USER_TABLE_NAME + ".secure_id AS userpost_sid, " + USER_TABLE_NAME + ".name AS userpost_name, " + USER_TABLE_NAME + ".surname AS userpost_surname, " + USER_TABLE_NAME + ".mail AS userpost_mail, " + USER_TABLE_NAME + ".phone AS userpost_phone, " + getTableName() + ".price AS post_price, " + CATEGORY_TABLE_NAME + ".name AS category_name, " + POST_STATE_TABLE_NAME + ".name AS poststate_name, " + getTableName() + ".date AS post_date FROM " + getTableName() + " " +
                    ...
                    "INNER JOIN " + CATEGORY_TABLE_NAME + " ON " + getTableName() + ".category = " + CATEGORY_TABLE_NAME + ".id " +
                    "INNER JOIN " + POST_STATE_TABLE_NAME + " ON " + getTableName() + ".state = " + POST_STATE_TABLE_NAME + ".id " +
                    "WHERE " + getTableName() + ".secure_id=?", List.of(new Tuple<>(postID, Types.VARCHAR)));
    ...
    private List<String> fetchPostImages(DatabaseRequest req, int postDatabaseID, String postSecureID, int limit) {
        ...
        try {
            final ResultSetWrappingSqlRowSet res = req.getValues(POST_IMG_TABLE_NAME, List.of("secure_id"), "post=?", List.of(new Tuple<>(postDatabaseID, Types.INTEGER)));
